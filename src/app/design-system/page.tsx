      ...
      <Container>
        <Stack gap="8" _paddingTop="48px" _paddingBottom="96px">
          ...
          <Box _paddingBottom="48px" _borderBottom="1px solid var(--rule)">
            ...
            <Text size="body-lg" color="muted" _marginTop="16px" _maxWidth="60ch">
              A complete inventory of every primitive and custom component in the system. All styles reference design tokens for single-source-of-truth theming.
            </Text>
          </Box>
          ...
          <Row columns={6} gap="2">
            {[
              ...
              { name: "surface-1", bg: "var(--surface-1)", color: "var(--text)" },
              { name: "surface-2", bg: "var(--surface-2)", color: "var(--text)" },
              { name: "surface-3", bg: "var(--surface-3)", color: "var(--text)" },
              { name: "surface-4", bg: "var(--surface-4)", color: "var(--text)" },
            ].map((s) => (
              <Box key={s.name} _background={s.bg} _color={s.color} _height="80px" _borderRadius="var(--r-xs)" _padding="10px 12px" _display="flex" _flexDirection="column" _justifyContent="space-between" _fontSize="10px" _border={s.border ? "1px solid var(--hairline)" : undefined}>
          ...
          <Row columns={6} gap="2">
            {[
              ...
              { name: "danger", bg: "var(--danger)", color: "var(--danger-on)" },
              { name: "warn", bg: "var(--warn)", color: "var(--warn-on)" },
              { name: "info", bg: "var(--info)", color: "var(--info-on)" },
              { name: "success", bg: "var(--success)", color: "var(--success-on)" },
            ].map((s) => (
              <Box key={s.name} _background={s.bg} _color={s.color} _height="80px" _borderRadius="var(--r-xs)" _padding="10px 12px" _display="flex" _flexDirection="column" _justifyContent="space-between" _fontSize="10px">
                <Text size="micro" _color="inherit">{s.name}</Text>
              </Box>
            ))}
          </Row>
          ...
          <Row columns={3} gap="4">
            ...
            <Card variant="frost">
              <Heading variant="subheading" level={3}>Frost</Heading>
              <Text size="caption" color="muted" _marginTop="8px">Frosted glass backdrop blur.</Text>
            </Card>
            <Card variant="block">
              <Heading variant="subheading" level={3}>Block</Heading>
              <Text size="caption" color="muted" _marginTop="8px">Accent bar indicator.</Text>
            </Card>
          </Row>
          ...
          <Row columns={2} gap="4">
            <Stack gap="4">
              ...
              <Field>
                <Field.Label htmlFor="ds-email" required>Email</Field.Label>
                <Field.Input
                  id="ds-email"
                  type="email"
                  ...
                  error={emailTouched && emailVal.length > 0 && !emailVal.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)}
                />
                {emailTouched && emailVal.length > 0 && !emailVal.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/) && (
              ...
              <Select value={formSelectVal} onChange={setFormSelectVal} placeholder="Select option">
            ...
            <Stack gap="4">
              ...
              <Card variant="default" padding="sm">
                <Text size="micro" color="dim" _marginBottom="12px">CheckboxGroup</Text>
                <CheckboxGroup value={checkVals} onChange={setCheckVals}>
                  ...
                  <CheckboxGroup.Item value="three" disabled>Disabled</CheckboxGroup.Item>
                </CheckboxGroup>
              </Card>
              <Stack direction="horizontal" gap="4" align="center">
                <Switch checked={switchA} onChange={setSwitchA}>Enabled</Switch>
                <Switch checked={switchB} onChange={setSwitchB}>Disabled</Switch>
              </Stack>
            </Stack>
          </Row>
          ...
          <Row columns={2} gap="4">
            <Card variant="default" padding="sm">
              ...
              <Select value={selectVal} onChange={setSelectVal} placeholder="Choose a framework">
                <Select.Option value="next" icon={<Icon name="arrow" />}>Next.js</Select.Option>
                <Select.Option value="remix" icon={<Icon name="diamond" />}>Remix</Select.Option>
                <Select.Option value="astro" icon={<Icon name="star" />}>Astro</Select.Option>
                <Select.Option value="svelte" disabled>SvelteKit (N/A)</Select.Option>
              </Select>
              <Text size="caption" color="dim" _marginTop="8px">Selected: {selectVal || "none"}</Text>
            </Card>
            <Card variant="default" padding="sm">
              <Text size="micro" color="dim" _marginBottom="12px">Multi Select</Text>
              <MultiSelect value={multiSelectVal} onChange={setMultiSelectVal} placeholder="Select technologies">
                <MultiSelect.Option value="react">React</MultiSelect.Option>
                <MultiSelect.Option value="typescript">TypeScript</MultiSelect.Option>
                <MultiSelect.Option value="tailwind">Tailwind</MultiSelect.Option>
                <MultiSelect.Option value="node">Node.js</MultiSelect.Option>
              </MultiSelect>
              <Text size="caption" color="dim" _marginTop="8px">Selected: {multiSelectVal.join(", ") || "none"}</Text>
            </Card>
          </Row>
          ...
          <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
            ...
            <Modal.Description>This action cannot be undone. Are you sure you want to proceed?</Modal.Description>
            <Modal.Actions>
          ...
          <Stack gap="3">
            <Alert severity="info" title="Information">Informational message for the user.</Alert>
            <Alert severity="success" title="Success">Operation completed successfully.</Alert>
            <Alert severity="warn" title="Warning">Something needs your attention.</Alert>
            <Alert severity="danger" title="Error" dismissible>A critical error occurred.</Alert>
          </Stack>

          <Text size="micro" color="dim" _marginTop="16px" _marginBottom="8px">Notification System (useAlert)</Text>
          ...
          <Stack direction="horizontal" gap="3" align="center" wrap>
            ...
            <Avatar color="primary" round>AC</Avatar>
            <Avatar color="secondary" round presence>ON</Avatar>
          </Stack>
          ...
          <DataTable>
            ...
            <DataTable.Body>
              <DataTable.Row>
                ...
                <DataTable.Td><Badge color="success" dot>Complete</Badge></DataTable.Td>
